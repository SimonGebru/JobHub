...
export default function SavedPage() {
  ...
  const handleStatusChange = async (id: string, status: SavedStatus) => {
    try {
      const updated = await updateSavedJob(id, { status });
  ...
  const handleNotesChange = (id: string, notes: string) => {
    setJobs((prev) =>
      prev.map((job) => (job.id === id ? { ...job, notes } : job))
    );
    ...
    debounceTimers.current[id] = setTimeout(async () => {
      try {
        const updated = await updateSavedJob(id, { notes });
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast({
        title: "Fel",
        description: "Kunde inte ta bort jobbet",
        variant: "destructive",
      });
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <Tabs defaultValue="saved">
            ...
            {statusTabs.map((tab) => {
              ...
              return (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
                  className="rounded-lg data-[state=active]:shadow-sm px-4 py-2"
                >
                  {tab.label}
                  {count > 0 && (
                    <span
                      className={`ml-1.5 text-[11px] font-bold px-1.5 py-0.5 rounded-full ${tab.color}`}
                    >
                      {count}
                    </span>
                  )}
          ...
          {statusTabs.map((tab) => {
            const jobsInTab = sortedByDate(
              jobs.filter((job) => job.status === tab.value)
            );
            ...
            return (
              ...
              >
                {jobsInTab.map((job) => (
                  <div
                    key={job.id}
                    className="border border-border/60 rounded-xl p-5 bg-card space-y-3 animate-fade-in"
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <select
                          value={job.status}
                          onChange={(e) =>
                            handleStatusChange(job.id, e.target.value as SavedStatus)
                          }
                    ...
                    <div>
                      ...
                      <TagInput
                        tags={allTags[job.jobId] || []}
                        onChange={(tags) => handleTagsChange(job.jobId, tags)}
                      />
                    </div>
                  ...
                  <div className="py-16 text-center">
                    ...
                    {tab.value === "saved" && jobs.length === 0 && (
                      <p className="text-sm text-muted-foreground/70 mt-1">
                        ...
                        och spara jobb du är intresserad av
                      </p>
                    )}
                  </div>
                )}
